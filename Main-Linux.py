...
DEFAULT_TEMPLATE_PPTX_PATH = "C:/Users/2399586/VScode/ExcelToPpt/Files/Default_Template.pptx"
...
def format_main_title(shape, last_week):
    ...
    if not (shape and shape.has_text_frame):
        return
...
def get_or_create_slide(prs: Presentation, slide_index: int, layout_index: int = 0):
    """
    Ensure presentation has a slide at slide_index; if not, add slides using the specified layout.
    Returns the slide at slide_index.
    """
        ...
        for cell in row.cells:
            for para in cell.text_frame.paragraphs:
                ...
                for run in para.runs:
                    run.font.name = "Aptos"
                    run.font.size = Pt(11)
                    run.font.bold = (row_idx == 0 or is_total_row)

def _populate_table_slide6(table_shape, table_data: TableData):
    if not (table_shape and table_shape.has_table):
        return
...
def populate_slide_6(prs, df, slide_index):
    ...
    slide = get_or_create_slide(prs, slide_index)
    ...
    if title_shape:
        format_main_title(title_shape, inc_data_obj.headers[-1])
...
if __name__ == "__main__":
    ...
    EXCEL_FILE_PATH = os.environ.get("EXCEL_FILE_PATH", 'C:/Users/2399586/VScode/ExcelToPpt/Files/ExcelData27.xlsx')
    TEMPLATE_PPTX_PATH = os.environ.get("TEMPLATE_PPTX_PATH") or DEFAULT_TEMPLATE_PPTX_PATH
    FINAL_OUTPUT_PPTX_PATH = os.environ.get("FINAL_OUTPUT_PPTX_PATH", 'C:/Users/2399586/VScode/ExcelToPpt/Outputs/Final_Output.pptx')
    ...
    print(f"Using FINAL_OUTPUT_PPTX_PATH = {FINAL_OUTPUT_PPTX_PATH}")
    ...
    # Load template (if provided and exists), otherwise use blank Presentation()
    prs = None
    try:
